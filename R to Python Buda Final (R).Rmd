...
Credit$Education_t <- cut(Credit$Education, breaks = c(5, 13, 21), labels = c("low education", "high education"), right = FALSE)
...
```
```{r}
residualPlots(creditm1, ask = FALSE)
...
```{r}
residualPlots(creditm2, ask = FALSE)
...
```{r}
residualPlots(creditm3, ask = FALSE)
...
```{r}
residualPlots(creditm4, ask = FALSE)
...
```
```{r}
Anova(creditm4, type = 'II')
```
```{r}
avPlots(creditm4, ask = FALSE)
```
...
```
```{r}
residualPlots(creditpre, ask = FALSE)
...
```
```{r}
residualPlots(credittrue, ask = FALSE)
...
```
```{r}
residualPlots(credittrue2, ask = FALSE)
...
y <- Credit$has_bal
p_te <- predict(creditbin, newdata = Credit, type = 'response')
y <- as.numeric(y)
p_te <- as.numeric(p_te)
p_te <- pmin(pmax(p_te, 1e-12), 1 - 1e-12)
l_p <- log(p_te/(1 - p_te))
fit <- glm(y~l_p, family = binomial())
 
intercept <- unname(fit$coefficients[1])
slope <- unname(fit$coefficients[2])
 
print(paste("Calibration Intercept:", intercept))
print(paste("Calibration Slope:", slope))
```
...
```{r}
set.seed(300)
Credit_split <- initial_split(Credit, prop = .80)
Credit_train <- training(Credit_split)
Credit_test <- testing(Credit_split)

creditbinmini <- glm(has_bal~Income+Limit+Cards+Age+Education+Gender+Married+Ethnicity, family = 'binomial', data = Credit_train)
pred_test <- predict(creditbinmini, newdata = Credit_test, type = 'response')
brier <- mean((pred_test - Credit_test$has_bal)^2)

pres2 <- predict(creditbinmini, newdata = Credit_test, type = 'response')
pres2 <- as.numeric(pres2)
pres2 <- pmin(pmax(pres2, 1e-12), 1 - 1e-12)
log_pres2 <- log(pres2/(1 - pres2))
fit <- glm(has_bal~log_pres2, family = 'binomial', data = Credit_test)
...
```{r}
K <- 10
breaks <- quantile(pred_test, probs = seq(0, 1, length.out = K +1), na.rm = TRUE)
breaks[1] <- -Inf; breaks[length(breaks)] <- Inf

bin <- cut(pred_test, breaks = breaks, include.lowest = TRUE)

cal_df <- aggregate(
  cbind(mean_pred = pred_test, frac_pos = Credit_test$has_bal) ~ bin,
  FUN = mean
)

plot(cal_df$mean_pred, cal_df$frac_pos, xlab = "Mean Predicted Probability", ylab= "Observed Fraction Positive", pch = 19)
lines(cal_df$mean_pred, cal_df$frac_pos)
